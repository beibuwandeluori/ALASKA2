...
from network.model import REC_EfficientNet
from dataset.dataset import DatasetRetriever, get_train_transforms, get_valid_transforms, shuffle_two_tensor
...
def train_model(model, criterion, optimizer, epoch, is_shuffle=False, lam=0.5):
    ...
    for i, (XI, label) in enumerate(train_loader):
        ...
        y_pred, rec_img = model(x)
        # Compute and print loss
        clf_loss = criterion(y_pred, label)
        rec_loss = nn.functional.l1_loss(rec_img, x)
        loss = clf_loss+ lam * rec_loss
        ...
        clf_loss_aver.append(clf_loss.item())
        rec_loss_aver.append(rec_loss.item())
        if i % 500 == 1:
            print('%s train %s images, use %s seconds, total loss %s, clf loss: %s, rec loss: %s\n' %
                  (i, i * batch_size, time.time() - start, sum(loss_aver) / len(loss_aver) if len(loss_aver) > 0 else 'NoLoss',
                   sum(clf_loss_aver) / len(clf_loss_aver) if len(clf_loss_aver) > 0 else 'NoLoss',
                   sum(rec_loss_aver) / len(rec_loss_aver) if len(rec_loss_aver) > 0 else 'NoLoss'))
        if i % 1000 == 1:
            with open(writeFile, 'a') as outF:
                outF.write('%s train %s images, use %s seconds, total loss: %s, clf loss: %s, rec loss: %s\n' %
                           (i, i*batch_size, time.time() - start, sum(loss_aver) / len(loss_aver) if len(loss_aver) > 0 else 'NoLoss',
                            sum(clf_loss_aver) / len(clf_loss_aver) if len(clf_loss_aver) > 0 else 'NoLoss',
                            sum(rec_loss_aver) / len(rec_loss_aver) if len(rec_loss_aver) > 0 else 'NoLoss'))
...
if __name__ == '__main__':
    ...
    writeFile = 'output/logs/rec_efficientnet/' + model_name + '_4c.out'  # DCT_YCrCb
    store_name = 'output/weights/rec_efficientnet/' + model_name + '_4c.pth'
    # model_path = None
    model_path = '/data1/cby/py_project/ALASKA2/output/weights/rec_efficientnet/efficientnet-b2_4c.pth43'

    # Load model
    # model = get_efficientnet(model_name=model_name)
    model = REC_EfficientNet(num_classes=4,
                             model_path='output/weights/efficientnet/efficientnet-b2_4c.pth64')
    ...
    if is_train:
        ...
        xdl = DatasetRetriever(is_one_hot=True, data_type='train', transforms=get_train_transforms(), is_mix_up=False)
        ...
        xdl_eval = DatasetRetriever(is_one_hot=False, data_type='val', transforms=get_valid_transforms(), is_mix_up=False)
        ...
        epoch_start = 1
        num_epochs = 1
        xdl_test = DatasetRetriever(data_type='val', transforms=get_valid_transforms(), is_one_hot=False,
                                    classes_num=4, QF=95)
