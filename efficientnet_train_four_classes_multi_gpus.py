...
if __name__ == '__main__':
    ...
    writeFile = 'output/logs/efficientnet/' + model_name + '_4c.out'  # DCT_YCrCb
    store_name = 'output/weights/efficientnet/' + model_name + '_4c.pth'
    model_path = None
    ...
    if is_train:
        ...
        xdl = DatasetRetriever(is_one_hot=False, data_type='train', transforms=get_train_transforms())
        train_loader = torch.utils.data.DataLoader(xdl, batch_size=batch_size, shuffle=False, num_workers=4,
                                                   sampler=BalanceClassSampler(labels=xdl.get_labels(), mode="downsampling"))
        train_dataset_len = len(xdl)

        xdl_eval = DatasetRetriever(is_one_hot=False, data_type='val', transforms=get_valid_transforms())
        eval_loader = torch.utils.data.DataLoader(xdl_eval, batch_size=test_batch_size, shuffle=False,
                                                  num_workers=4)
        ...
        test_loader = torch.utils.data.DataLoader(xdl_test, batch_size=test_batch_size, shuffle=False, num_workers=4)
