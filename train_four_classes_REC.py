...
from network.model import model_selection, get_efficientnet, DCT_EfficientNet, REC_EfficientNet
...
def train_model(model, criterion, optimizer, epoch, is_shuffle=False, lam=0.001):
...
if __name__ == '__main__':
    ...
    writeFile = 'output/logs/rec_efficientnet_YCrCb/' + model_name + '_YCrCb_4c.out'  # DCT_YCrCb
    store_name = 'output/weights/rec_efficientnet_YCrCb/' + model_name + '_YCrCb_4c.pth'
    # model_path = None
    model_path = 'output/weights/rec_efficientnet_YCrCb/efficientnet-b2_YCrCb_4c.pth19'
    ...
    if is_train:
        ...
        xdl = AlaskaDataset(data_type='train', classes_num=4, is_YCrCb=is_YCrCb,
                            is_albumentations=is_albumentations, is_one_hot=True)
        ...
        xdl_eval = AlaskaDataset(data_type='val', classes_num=4, is_YCrCb=is_YCrCb,
                                 is_albumentations=is_albumentations, is_one_hot=False)
        ...
        epoch_start = 1
        num_epochs = 1
        xdl_test = AlaskaDataset(data_type='val', is_four_classes=True)
