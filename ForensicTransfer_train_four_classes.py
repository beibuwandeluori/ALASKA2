...
from network.ForensicTransfer import ForensicTransfer
...
def train_model(model, criterion, optimizer, epoch, is_shuffle=False, lam=0.5):
    ...
    for i, (XI, label) in enumerate(train_loader):
        ...
        loss = criterion(y_pred, label) + lam * nn.functional.l1_loss(rec_img, x)
...
if __name__ == '__main__':
    ...
    lr = 0.002
    # model_name = 'resnet18'
    model_name = 'ForensicTransfer'
    writeFile = 'output/logs/ForensicTransfer/' + model_name + '_4c.out'  # DCT_YCrCb
    store_name = 'output/weights/ForensicTransfer/' + model_name + '_4c.pth'
    model_path = None

    # Load model
    # model = get_efficientnet(model_name=model_name)
    model = ForensicTransfer(num_classes=4)
    ...
    if is_train:
        ...
        best_weight_auc = 0.9204 if epoch_start == 0 else eval_model(model, epoch_start, eval_loader)
