...
def get_images_by_QF(QF, image_paths_npy='/pubdata/chenby/py_project/ALASKA2/dataset/QF_npy/train_all_image_names.npy',
                     QFs_npy='/pubdata/chenby/py_project/ALASKA2/dataset/QF_npy/train_all_QFs.npy'):
...
def select_data_by_QF(QF, image_paths, labels,
                      image_paths_npy='/pubdata/chenby/py_project/ALASKA2/dataset/QF_npy/train_all_image_names.npy',
                      QFs_npy='/pubdata/chenby/py_project/ALASKA2/dataset/QF_npy/train_all_QFs.npy'):
...
def read_data_from_csv(root_path='/pubdata/chenby/dataset/alaska2/', data_type='train'):

    # ['Normal', 'JMiPOD_75', 'JMiPOD_90', 'JMiPOD_95', 'JUNIWARD_75', 'JUNIWARD_90',
    #  'JUNIWARD_95', 'UERD_75', 'UERD_90', 'UERD_95']

    if data_type == 'train':
        data_df = pd.read_csv('/pubdata/chenby/py_project/ALASKA2/dataset/csv_files/alaska2_train_df.csv')
    else:
        data_df = pd.read_csv('/pubdata/chenby/py_project/ALASKA2/dataset/csv_files/alaska2_val_df.csv')
...
class AlaskaDataset(Dataset):
    def __init__(self, data_type='train', is_shuffle=False, is_YCrCb=False, classes_num=2,
                 is_albumentations=False, is_one_hot=False, use_one_cover_type=3):
...
if __name__ == '__main__':
    ...
    img = read_image_yo_YCrCb(image_path=ROOT_PATH + 'Cover/00001.jpg', branches='YCrCb', data_type='train')
    print(img.shape)

    # start = time.time()
    # xdl = AlaskaDataset(data_type='train', is_shuffle=False, is_YCrCb=True, is_albumentations=False,
    #                     classes_num=2, is_one_hot=True)
    ...
    # start = time.time()
    # xdl = DatasetRetriever(is_one_hot=False, transforms=get_train_transforms(), classes_num=4, QF=95)
    # print('length:', len(xdl))
    # train_loader = DataLoader(xdl, batch_size=20, shuffle=False, num_workers=1,
    #                           )  # sampler=BalanceClassSampler(labels=xdl.get_labels(), mode="downsampling")
    # for i, (img, label) in enumerate(train_loader):
    #     print(i, img.shape, label.size(), label)
    #     if i == 10:
    #         break
    #     # break
    #
